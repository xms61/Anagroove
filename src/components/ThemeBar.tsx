...
export const ThemeBar: React.FC<ThemeBarProps> = ({
  ...
  return (
    <div className="w-full bg-kissa-base/85 border-b border-white/10 px-4 py-2.5 backdrop-blur-md sticky top-[57px] z-20 overflow-x-auto no-scrollbar shadow-sm">
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-semibold transition-all shrink-0 cursor-pointer ${
                  isActive
                    ...
                    : 'bg-kissa-card/70 hover:bg-kissa-panel text-slate-300 hover:text-white border border-white/5'
                }`}
              >
                <span>{t.icon}</span>
                <span>{t.name}</span>
