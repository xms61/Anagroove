...
import { createTrackPicker } from '../../server/selection/trackPicker.ts';
import type { SongCandidate } from '../../server/types.ts';

/** Test songs get an id from their artist and title. */
const withIds = (songs: (Partial<SongCandidate> & { title: string; artist: string })[]): SongCandidate[] => songs.map(song => ({ id: `${song.artist}|${song.title}`, ...song }));
...
test('tracks played more often than maxPlays wait for a later tier', () => {
  ...
  const candidates = withIds([{ title: 'Wonderwall', artist: 'Oasis', language: 'en' }, { title: 'Creep', artist: 'Radiohead', language: 'en' }]);
  pick(candidates, 1);
