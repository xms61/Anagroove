...
- Versions are tracked in `PRAGMA user_version` (currently **v2**). Each migration runs in its own transaction.
...
- `tracks`:
  ...
  - `canonical_title`: the Unicode **base title** key from `baseTitleKey`. Credits and version tags are removed; kana, hangul and kanji are kept.
  ...
  - `language`: detected; `country_code` is the ISRC **registrant** prefix, not a language.
...
- **Language** `en`, `ja` or `ko` (`detectTrackLanguage`; Han-only text counts as `ja`/`ko` only with a JP/KR ISRC).
